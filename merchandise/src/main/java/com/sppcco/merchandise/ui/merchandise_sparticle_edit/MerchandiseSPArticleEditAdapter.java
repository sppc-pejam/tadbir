...
public class MerchandiseSPArticleEditAdapter extends RecyclerView.Adapter<MerchandiseSPArticleEditAdapter.ViewHolder> {
  ...
  MerchandiseSPArticleEditAdapter(
    MerchandiseSPArticleEditContract.View view,
    MerchandiseSPArticleEditContract.Presenter presenter) {
    mView = view;
  ...
  public void onBindViewHolder(@NonNull final ViewHolder viewHolder, int position) {
    showImages(viewHolder.imgMerchandise, viewHolder.prgLoading, mPresenter.getSPArticle().getMerchandiseId(), position);


    viewHolder.imgMerchandise.setOnClickListener(v -> {
      if (getImageIds().size() != 0) {
        mView.callImageGallery(mPresenter.getSPArticle().getMerchandiseId(), getImageIds(), position);
