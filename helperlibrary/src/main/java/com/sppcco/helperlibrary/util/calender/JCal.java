...
public class JCal {
  ...
  /**
   * Create a ir.huri.jcal.JCal object
   * @param year Jalali Year
   * @param month Jalali Month
   * @param day Jalali Day
   */
  public JCal(int year, int month, int day) {
    set(year, month, day);
  }


  /**
   * Create a ir.huri.jcal.JCal object from gregorian calendar
   * @param gc gregorian calendar object
   */
  public JCal(GregorianCalendar gc){
    fromGregorian(gc);
  }


  /**
   * Create a ir.huri.jcal.JCal object from Localdate(java 8)
   * @param ld local date object
   */
  @SuppressLint("NewApi")
  public JCal(LocalDate ld) {
    fromGregorian(GregorianCalendar.from(ld.atStartOfDay(ZoneId.systemDefault())));
  }

  /**
   * Create a ir.huri.jcal.JCal object from Date object
   * @param date Date object
   */
  public JCal(Date date) {
   ...
   */
  public JCal getDateByDiff(int diff) {
  ...
  private void fromJulianDay(int JulianDayNumber) {
    ...
    GregorianCalendar gregorianFirstFarvardin = new JCal(jalaliYear, 1, 1).getGregorianFirstFarvardin();
